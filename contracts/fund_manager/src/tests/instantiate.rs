...
use valkyrie::test_constants::fund_manager::{ADMINS, fund_manager_env, MANAGING_TOKEN, CAMPAIGN_ADD_POOL_FEE_BURN_RATIO_PERCENT, CAMPAIGN_ADD_POOL_FEE_RECIPIENT};
...
pub fn exec(
    ...
    campaign_add_pool_fee_burn_ratio: Decimal,
    campaign_add_pool_fee_recipient: String,
...
pub fn default(deps: &mut CustomDeps) -> (Env, MessageInfo, Response) {
    ...
    let response = exec(
        ...
        Decimal::percent(CAMPAIGN_ADD_POOL_FEE_BURN_RATIO_PERCENT),
        CAMPAIGN_ADD_POOL_FEE_RECIPIENT.to_string(),
    ).unwrap();
...
fn succeed() {
    ...
    assert_eq!(config, ContractConfig {
        ...
        campaign_add_pool_fee_burn_ratio: Decimal::percent(CAMPAIGN_ADD_POOL_FEE_BURN_RATIO_PERCENT),
        campaign_add_pool_fee_recipient: Addr::unchecked(CAMPAIGN_ADD_POOL_FEE_RECIPIENT),
    });
