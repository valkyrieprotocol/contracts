...
use crate::executions::receive_campaign_deposit_fee;
...
pub fn exec(
    ...
    receive_campaign_deposit_fee(deps.as_mut(), env, info, amount)
}
...
fn succeed() {
    ...
    assert_eq!(state.campaign_deposit_fee_amount, Uint128::new(33));

    will_success(&mut deps, Uint128::new(20));

    let state = ContractState::load(&deps.storage).unwrap();
    assert_eq!(state.campaign_deposit_fee_amount, Uint128::new(53));
}
