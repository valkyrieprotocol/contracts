...
fn succeed() {
    ...
    config.add_pool_fee_rate = Decimal::percent(1);
    deps.querier.with_global_campaign_config(config);

    let (env, info, response) = will_success(
        &mut deps,
        100 - ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT as u128,
        ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT as u128,
    );
    assert_eq!(response.messages, vec![
        SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::TransferFrom {
                ...
                amount: Uint128::from(ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT),
            }).unwrap(),
        })),
        SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: VALKYRIE_TOKEN.to_string(),
...
fn failed_zero_referral_token() {
    ...
    expect_generic_err(&result, format!(
        ...
        Decimal::percent(ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT).to_string(),
    ).as_str());
}
...
fn failed_insufficient_referral_token() {
    ...
    let info = mock_info(CAMPAIGN_ADMIN, &[
        coin(100 - ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT as u128, PARTICIPATION_REWARD_DENOM_NATIVE),
    ]);
    ...
    let result = exec(
        ...
        info,
        Uint128::from(100 - ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT),
        Uint128::from(ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT - 1),
    );
    expect_generic_err(&result, format!(
        "Referral reward rate must be greater than {}",
        Decimal::percent(ADD_POOL_MIN_REFERRAL_REWARD_RATE_PERCENT).to_string(),
...
fn test_deposit_fee_amount() {
    let result = calc_add_pool_fee_amount(
        Uint128::new(20),
        Decimal::percent(20),
        Decimal::percent(1),
    ).unwrap();
