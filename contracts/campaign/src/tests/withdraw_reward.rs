...
use crate::executions::withdraw_reward;
use valkyrie::test_utils::{contract_env, default_sender, expect_unauthorized_err, expect_generic_err};
use crate::tests::{campaign_admin_sender, TOKEN_CONTRACT, CAMPAIGN_ADMIN, BURN_CONTRACT, CAMPAIGN_DISTRIBUTION_AMOUNTS, CAMPAIGN_DISTRIBUTION_DENOM_NATIVE};
...
pub fn exec(
    ...
    let response = withdraw_reward(deps.as_mut(), env, info, denom, amount)?;
...
fn succeed_at_active() {
    ...
    deps.querier.with_valkyrie_config(1000, burn_rate.clone());
    ...
    assert_eq!(response.messages, vec![
        CosmosMsg::Bank(BankMsg::Send {
            to_address: BURN_CONTRACT.to_string(),
    ...
    assert_eq!(response.messages, vec![
        CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: TOKEN_CONTRACT.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Transfer {
                recipient: BURN_CONTRACT.to_string(),
                amount: burn_amount,
            }).unwrap(),
            send: vec![],
        }),
