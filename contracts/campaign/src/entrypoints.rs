...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateDistributionConfig {
            denom,
            amounts,
...
pub fn query(
    ...
    let result = match msg {
        QueryMsg::CampaignInfo {} => to_binary(
            &crate::queries::get_campaign_info(deps, env)?
        ),
        QueryMsg::DistributionConfig {} => to_binary(
            &crate::queries::get_distribution_config(deps, env)?
        ),
        QueryMsg::CampaignState {} => to_binary(
            &crate::queries::get_campaign_state(deps, env)?
        ),
        QueryMsg::ShareUrl { address } => to_binary(
            &crate::queries::get_share_url(deps, env, address)?
        ),
        QueryMsg::GetAddressFromReferrer { referrer } => to_binary(
            &crate::queries::get_address_from_referrer(deps, env, referrer)?
        ),
        QueryMsg::Participation { address } => to_binary(
            &crate::queries::get_participation(deps, env, address)?
        ),
        QueryMsg::Participations {
            ...
            &crate::queries::query_participations(deps, env, start_after, limit, order_by)?
        )
    }?;
